...
class TestApplyEncap(unittest.TestCase):
    ...
    def test_geneve_outer_udp_and_header(self):
        pkt = _build_with_encap(GeneveEncap(
            vni=5000, src_ip="203.0.113.1", dst_ip="203.0.113.2"))
        # Outer IP protocol = 17 (UDP) at byte 23
        self.assertEqual(pkt[23], 17)
        # Outer UDP dst port at 36
        self.assertEqual(struct.unpack_from("!H", pkt, 36)[0], GENEVE_PORT)
        # GENEVE base header at 42; Protocol Type (TEB) at +2 = 44
        self.assertEqual(struct.unpack_from("!H", pkt, 44)[0], 0x6558)
        # VNI in high 24 bits of the second word (offset 46)
...
class TestTCPStreamWithEncap(unittest.TestCase):
    ...
    def test_geneve_stream(self):
        s = self._stream(GeneveEncap(vni=42, src_ip="192.168.1.1", dst_ip="192.168.1.2"))
        pkt = s.packets[0].raw
        self.assertEqual(pkt[23], 17)  # outer UDP
        self.assertEqual(struct.unpack_from("!H", pkt, 36)[0], GENEVE_PORT)
