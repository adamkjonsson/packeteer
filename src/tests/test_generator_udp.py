...
class TestUDPHeader(unittest.TestCase):
    def test_length(self):
        raw = _build_udp_header(UDPHeader(1234, 53), b'', "1.2.3.4", "5.6.7.8")
        self.assertEqual(len(raw), 8)

    def test_udp_length_field(self):
        payload = b'\x00' * 10
        raw = _build_udp_header(UDPHeader(1234, 53), payload, "1.2.3.4", "5.6.7.8")
        length = struct.unpack('!H', raw[4:6])[0]
        self.assertEqual(length, 18)

    def test_checksum_v4(self):
        payload = b'query'
        raw = _build_udp_header(UDPHeader(5000, 53), payload, "192.168.1.1", "8.8.8.8")
        self.assertEqual(_verify_udp_checksum_v4("192.168.1.1", "8.8.8.8", raw, payload), 0)

    def test_checksum_v6(self):
        payload = b'data'
        raw = _build_udp_header(UDPHeader(5000, 53), payload, "::1", "::2", ip_version=6)
        self.assertEqual(_verify_udp_checksum_v6("::1", "::2", raw, payload), 0)

    def test_zero_payload(self):
        raw = _build_udp_header(UDPHeader(1000, 2000), b'', "1.2.3.4", "5.6.7.8")
        self.assertEqual(len(raw), 8)
        length = struct.unpack('!H', raw[4:6])[0]
