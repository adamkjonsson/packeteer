...
- **The generated receiver acknowledged a corrupted segment before its
  retransmission.**  `payload_corruption_probability` corrupts a segment
  after the connection is emitted and moved exactly one acknowledgement
  behind the clean resend; every later acknowledgement was cumulative and
  covered the corrupted range, so a consumer reading ack numbers — a policy
  keeping the copy the receiver acknowledged, a SACK or loss inference, a
  sequenced release — was told the receiver had kept bytes it rejected.  The
  moved acknowledgement also kept the original's number alone, lower than
  the ones before it: an ack number that went backwards.  The corruption pass
  now runs the same receiver `emit()` applies to a lost segment over the
  finished timeline: while the hole is open the receiver repeats its start
  and echoes the last in-order TSval, and the acknowledgement after the
  retransmission jumps past everything held and echoes the retransmission,
  as `tcp_corrupt_ts.pcap` shows a real receiver doing.  That
  acknowledgement is now labelled `ACK-RECOVER[n]`, as the loss path names
  its equivalent, rather than `ACK[n]`.  **Generated bytes change** for any
  seeded stream with `payload_corruption_probability` set.  (#163)

- **A recovering acknowledgement stalled at a sequence-number wrap.**  The
  loss path's `_contiguous_ack` compared sequence numbers as plain integers,
...
- **A retransmission carried the original's acknowledgement number.**  A
  resend was rebuilt with a fresh clock and a current TSecr but kept the
  `ack` of the segment it repeated, so one that went out after the peer's
  FIN had been acknowledged acknowledged one byte less than the segment
  before it — an ack number going backwards, on every seed, on the loss,
  retransmission and corruption paths alike.  A resend now acknowledges what
  its sender had received from the peer by the moment it was resent, as a
  real stack fills in `RCV.NXT` on every segment it builds.  Without
  timestamps a copy is still verbatim when nothing arrived from the peer in
  between; otherwise it differs in the acknowledgement field and checksum.
  The same walk now counts the sequence number a SYN or FIN consumes, so the
  `ACK-RECOVER` after a retransmission also covers a FIN the receiver had
  been holding behind the hole.  **Generated bytes change** for any seeded
  stream with retransmissions, however caused.  (#164)
